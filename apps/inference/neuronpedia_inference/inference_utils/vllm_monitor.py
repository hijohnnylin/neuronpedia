...
class VLLMMonitor:
    ...
    def __init__(self, model=None):
        ...
        Args:
            model: Optional VLLMSteerModel instance
        """
        self._model = model
    ...
    def _get_gpu_stats(self) -> list[GPUStats]:
        ...
        try:
            ...
            for i in range(device_count):
                ...
                # pynvml's stubs widen these to str|bytes|Any; they are byte counts.
    ...
    def _get_vllm_engine_stats_sync(self) -> dict[str, Any]:
        """Get vLLM engine internal stats (basic info only).

        Deep vLLM-engine cache stats came from steerllm's ``._inner._engine``, which the
        engine-owned ``VLLMModel`` does not expose, so only defaults are returned
        here (the basic GPU/system stats are collected separately). For detailed vLLM
        stats, use vLLM's built-in ``/metrics`` endpoint.
        """
        return {
            "initialized": False,
            "model_name": "",
    ...
    def _collect_all_blocking_stats(
        self,
    ) -> tuple[
        list[GPUStats],
    ...
    def start_background_logging(self, interval: float = 10.0):
        ...
        try:
            loop = asyncio.get_running_loop()
            self._background_task = loop.create_task(self._background_logging_loop(interval))
